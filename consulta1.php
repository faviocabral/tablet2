...
header('Content-Type: text/html; charset=UTF-8');  
	...
	$sucursal = 'victoria';
	$funcion = 	'ConsultaCampanha';	//'ConsultarOt';//'ConsultaCampanha';
	$vin = 'NU_3KPA341ABJE010674';
	...
	}elseif ($sucursal == '002' || $sucursal == '003' ){//nissan
...
/*
		...
		*/
		...
		$consulta = "
						select top 20 cardcode Codigo, cardname Nombre , isnull( phone1, '') Telefono1, isnull( address,'') Direccion, lictradnum Ruc 
...
//		echo json_encode( $valor, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP ); //fco esta linea codifica para ser leido como json 
		...
		//filtro sucursal 
		if ($sucursal == 'alider'){
			$filtro = " and id_sucursal = 2 "; //alider 
		}elseif ( $sucursal == 'victoria'){
			$filtro= " and id_sucursal in ( 1) "; //kia 

		}elseif ( $sucursal == 'choferes'){
			$filtro= " and id_sucursal in (3) "; //kia 
		}elseif ( $sucursal == '003'){
			$filtro= " and id_sucursal in (9) "; //nissan fernando 
			$base = 'reservas_fco';
		...
		*/		
		...
		if($sucursal == 'alider'){
			...
			$consulta = "select USERID Codigo, upper( U_NAME) Asesor from OUSR with(nolock) where Department = 1 and Branch = 2 ";
		}elseif($sucursal == '003'){
			$consulta = "select USERID Codigo, upper( U_NAME) Asesor from OUSR with(nolock) where Department = 1 and Branch = 4 ";
		}
		...
		$consulta = " 
					select t1.Cod_Camp CODIGO, ROW_NUMBER() OVER(ORDER BY VIN) FILA, vin VIN, upper( Modelo_camp ) MODELO, open_date FECHA, status ESTADO, upper( Desc_camp ) TRABAJO , upper( tipo ) TIPO 
	...
	select * 
					From ( 
						...
						select t1.CardCode codigo,
							...
							sum(case when t4.u_tipocuota = 'cuo' then 1 else 0 end ) cuotas,  
							sum(case when t4.Status = 'c' and t4.u_tipocuota = 'cuo' then 1 else 0 end ) pagado 
						From [sap-sql1].temasa.dbo.OINV t1 with(nolock) 
							inner join [sap-sql1].temasa.dbo.inv1 t2 with(nolock) on t1.docentry = t2.docentry 
							inner join [sap-sql1].temasa.dbo.oitm t3 with(nolock)	on t2.itemcode = t3.itemcode
							left outer join [sap-sql1].temasa.dbo.inv6 t4 with(nolock) on t1.docentry = t4.docentry 
